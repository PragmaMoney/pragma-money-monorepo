...
export const EXPLORER_BASE_URL =
  process.env.NEXT_PUBLIC_EXPLORER_BASE_URL || "https://monad-testnet.socialscan.io";
...
export function getExplorerTokenUrl(address: string, tokenId?: string | number): string {
  if (tokenId === undefined) {
    return `${EXPLORER_BASE_URL}/token/${address}`;
  }
  return `${EXPLORER_BASE_URL}/token/${address}?a=${tokenId}`;
}
