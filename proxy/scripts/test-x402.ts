 ...
 * 
 * Verified contracts deployed to Monad Testnet:
...
const AGENT_ID = process.env.AGENT_ID ? Number(process.env.AGENT_ID) : 1;
...
async function main() {
  ...
  // Step 7: Test replay protection (should fail with "Payment already used")
  console.log("🔁 Step 7: Testing replay protection (reusing same paymentId)...");
  try {
